...
./test-release.sh -release "$LLVM_RELEASE" -final -triple "$1"-apple-darwin21.0 -no-checkout -no-clang-tools -no-test-suite -no-polly -no-mlir -no-flang -no-compare-files -configure-flags -DLLVM_APPEND_VC_REV=OFF -"$2"
if [ "$2" != "stage3" ]; then
